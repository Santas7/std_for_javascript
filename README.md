...
метод atoi, который принимает строку в качестве аргумента и возвращает числовое значение, соответствующее этой строке.Метод parseInt использует цикл для прохода по символам строки. Если символ является цифрой, то он добавляется в конец числа. Если символ не является цифрой, цикл прерывается.
